...
def _generate_java_project_hierarchy(project_name, java_package_name, java_project_path):
    _mkdir(java_project_path, exist_ok=True)
    pom_path = os.path.join(java_project_path, FILE_POM)

    if not os.path.exists(pom_path):
        _LOG.info(f"Generating pom inside {java_project_path}")
        ...
        _LOG.info(f"Pom file inside {java_project_path} already exists")

    _mkdir(Path(java_project_path, SRC_MAIN_JAVA), exist_ok=True)
