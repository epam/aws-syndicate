...
from commons.checkers import exit_code_checker, artifacts_existence_checker, \
    ...
    api_gw_resources_existence_checker, api_gw_binary_media_types_checker
...
def api_gw_binary_media_types_handler(resources: dict, **kwargs) \
        -> bool | dict:
    invalid_conf = {}
    for api_gw_name, config in resources.items():
        api_gw_name = populate_prefix_suffix(api_gw_name,
                                             CONFIG.resources_prefix,
                                             CONFIG.resources_suffix)
        expected = config.get('binary_media_types')
        if expected is None:
            continue
        if result := api_gw_binary_media_types_checker(
                api_gw_name, expected):
            invalid_conf[api_gw_name] = result
...
HANDLERS_MAPPING = {
    ...
    'api_gw_binary_media_types': api_gw_binary_media_types_handler,
