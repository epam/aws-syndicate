...
from core import CONFIG, CONF_PATH, CONN, ClientError
from core.helper import create_pool, unpack_kwargs
from core.resources.helper import build_description_obj, chunks

_LOG = get_logger('core.resources.ec2_resource')
_EC2_CONN = CONN.ec2()
_IAM_CONN = CONN.iam()
...
@unpack_kwargs
def _create_ec2_from_meta(name, meta):
    # checking required parameters
    image_id = meta['image_id']
    image_data = _EC2_CONN.describe_image(image_id=image_id)
    if not image_data:
    ...
    if not _EC2_CONN.if_key_pair_exists(key_name):
    ...
    if availability_zone:
        ...
        subnet_list = _EC2_CONN.list_subnets(filters=[subnet_filter])
        if subnet and subnet not in \
                [subnet_ids['SubnetId'] for subnet_ids in subnet_list]:
            raise AssertionError('There is no available Subnets with name {0} '
                                 'in Availability Zone {1}.'
                                 .format(subnet, availability_zone))
        if availability_zone not in _EC2_CONN.get_azs():
            raise AssertionError('There is no Availability Zone with name: {0}'
                                 .format(availability_zone))

    security_groups_names = meta.get('security_group_names')
    if security_groups_names:
        sg_meta = _EC2_CONN.describe_security_groups(security_groups_names)
    ...
    if iam_role_name:
        instance_profiles = _IAM_CONN.get_instance_profiles_for_role(
    ...
    # launching instance
    response = _EC2_CONN.launch_instance(
        name=name,
        image_id=image_id,
    ...
    if meta.get('disableApiTermination'):
        ...
        if str(disable_api_termination).lower() == 'true':
            _EC2_CONN.modify_instance_attribute(
    ...
    return describe_ec2(name, meta, response)
...
def remove_instance_list(*instance_list):
    ...
    if existing_instances_list:
        _EC2_CONN.terminate_instances(instance_ids=existing_instances_list)
    _LOG.info('EC2 instances %s were removed.', str(existing_instances_list))
